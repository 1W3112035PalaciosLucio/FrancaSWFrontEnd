...
export class AltaOrdenesProduccionComponent implements OnInit {
  ...
  registrarOrden(f: NgForm) {
    if (f.valid) {
      // Validar si la fecha de pedido es anterior a la fecha de entrega
      const fechaPedido = new Date(this.ordenProd.fechaPedido);
      const fechaEntrega = new Date(this.ordenProd.fechaEntrega);

      if (fechaPedido >= fechaEntrega) {
        Swal.fire({
          title: "Error",
          text: "La fecha de pedido debe ser anterior a la fecha de entrega.",
          confirmButtonColor: '#162B4E'
        });
